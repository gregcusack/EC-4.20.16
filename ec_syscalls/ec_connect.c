...
SYSCALL_DEFINE4(ec_connect_, char*, GCM_ip, int, GCM_port, int, pid, int, ec_id) {
	printk(KERN_INFO "in ec_connect() sys call. ec_id: %d\n", ec_id);
	...
	if(ec_connect_) {
		...
		return ec_connect_(GCM_ip, GCM_port, pid, ec_id);
	}
